...
import { RESERVATION_URL } from "@/constants/common";
    ...
    <header className="fixed top-0 left-0 right-0 h-[96px] flex px-[37px] items-center justify-between bg-white/[0.95] z-50">
      <Link to="/">
        <h1>
          <img
            src={logo}
            alt="Urban Hair Studio"
            className="block w-[130.72px] h-[54.01px]"
          />
        </h1>
      </Link>
      <nav>
        <ul className="flex gap-[37px] text-[14px] leading-[28px] font-bold">
          {navItems.map((item) => {
            ...
            const borderStyle: string = isActive
              ? "border-b-2 border-[#1C1917]"
              : "";

            return (
              <li
                key={item.path}
                className="px-[10px] pt-[8px] rounded-[2px] transition-colors duration-[300ms] hover:bg-black/[0.051]"
              >
                <Link
                  to={item.path}
                  className={`block pb-[8px] text-[${textColor}] ${borderStyle}`}
                >
                  {item.name}
                </Link>
              </li>
          ...
          <li>
            <a
              className="flex items-center justify-center w-[185.7px] h-[44px] bg-[#1C1917] rounded-[16777200px] gap-[8px] shadow-lg"
            ...
            >
              <img
                src={calendar}
                alt="예약하기"
                className="block w-[18px] h-[18px] mr-[8px]"
              />
    ...
    <>
      <header className="fixed top-0 left-0 right-0 h-[59px] flex items-center justify-between p-[20px] bg-white z-50">
          ...
          {location.pathname === "/" ? (
            <h1>
              <img
                src={logo}
                alt="Urban Hair Studio"
                className="block w-[86.72px] h-[35.83px]"
              />
            </h1>
          ) : (
            <img
        ...
        <img
          ...
          alt="menu"
          className="block w-[24px] h-[24px] absolute right-[16px] cursor-pointer"
